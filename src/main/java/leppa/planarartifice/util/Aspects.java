...
public class Aspects extends AspectList implements Comparable<AspectList> {
    ...
    public static void register(Aspect ...aspect) {
        PlanarArtifice.LOGGER.info("[PA, ASPECT] Registering " + String.join(", ", Arrays.stream(aspect).map(Aspect::getTag).toArray(String[]::new)));
        for (Aspect a : aspect) compatAspects.put(a.getTag(), a);
    }
