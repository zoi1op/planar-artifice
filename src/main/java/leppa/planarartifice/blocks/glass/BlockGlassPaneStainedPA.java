...
public class BlockGlassPaneStainedPA extends BlockGlassPanePA {
    public static final PropertyEnum<EnumDyeColor> COLOR = PropertyEnum.create("color", EnumDyeColor.class);

    public BlockGlassPaneStainedPA(String name) { this(name, false, false, false); }
    public BlockGlassPaneStainedPA(String name, boolean passEntity, boolean passPlayer, boolean passHostile) {
    ...
    public void registerModels() {
        for (int i = 0; i < EnumDyeColor.values().length; ++i)
            ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(this), EnumDyeColor.values()[i].getMetadata(), new ModelResourceLocation(this.getRegistryName(), "color=" + EnumDyeColor.values()[i].getDyeColorName() + ",east=true,north=false,south=false,west=true"));
    }
}
